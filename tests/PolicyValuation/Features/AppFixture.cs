...
namespace PAS.PolicyValuation.Tests.Features;
...
public sealed class AppFixture : WebApplicationFactory<Program>, IAsyncLifetime {
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder) {
        ...
        builder.ConfigureServices(services => {
            // Replacing ValuationDbContext to use the db container connection string)
            var optionsDesc = services.SingleOrDefault(d => d.ServiceType == typeof(DbContextOptions<ValuationDbContext>));
            if (optionsDesc != null) services.Remove(optionsDesc);
            services.AddDbContext<ValuationDbContext>(options => options.UseSqlServer(DbConnectionString));

            var dbContextDesc = services.SingleOrDefault(d => d.ServiceType == typeof(ValuationDbContext));
            if (dbContextDesc != null) services.Remove(dbContextDesc);
            services.AddScoped<ValuationDbContext>();
        });
    }
    ...
    public async ValueTask InitializeAsync() {
        ...
        using (var bootstrapDbContext = new ValuationDbContext(optionsBuilder.Options)) {
    ...
    public async Task ExecuteDbContextAsync(Func<ValuationDbContext, Task> action) {
        using var scope = Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ValuationDbContext>();
        await action(db);
    }

    public async Task<T> ExecuteDbContextAsync<T>(Func<ValuationDbContext, Task<T>> action) {
        using var scope = Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<ValuationDbContext>();
