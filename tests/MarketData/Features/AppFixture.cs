...
public sealed class AppFixture : WebApplicationFactory<Program>, IAsyncLifetime {
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder) {
        ...
        builder.ConfigureServices(services => {
            // Replacing MarketDataDbContext to use the db container connection string)
            var optionsDesc = services.SingleOrDefault(d => d.ServiceType == typeof(DbContextOptions<MarketDbContext>));
            ...
            var dbContextDesc = services.SingleOrDefault(d => d.ServiceType == typeof(MarketDbContext));
            if (dbContextDesc != null) services.Remove(dbContextDesc);
            services.AddScoped<MarketDbContext>();
        });
    }
    ...
    public async ValueTask InitializeAsync() {
        ...
        using (var bootstrapDbContext = new MarketDbContext(optionsBuilder.Options)) {
    ...
    public async Task ExecuteDbContextAsync(Func<MarketDbContext, Task> action) {
        using var scope = Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<MarketDbContext>();
        await action(db);
    }

    public async Task<T> ExecuteDbContextAsync<T>(Func<MarketDbContext, Task<T>> action) {
        using var scope = Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<MarketDbContext>();
