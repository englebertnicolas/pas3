...
public static class MarketDataApiClientExtensions {

    public static IServiceCollection AddMarketDataApiClient(this IServiceCollection services, string serviceName = "api-marketdata") {
        services
            .AddHttpClient<MarketDataApiClient>(client => {
                client.BaseAddress = new Uri($"http://{serviceName}");
            })
            .AddTypedClient((httpClient, sp) => {
                var authProvider = new AnonymousAuthenticationProvider();
                var requestAdapter = new HttpClientRequestAdapter(authProvider, httpClient: httpClient);
                return new MarketDataApiClient(requestAdapter);
            });

        return services;
    }

    public static async Task<ErrorOr<T>> ToErrorOrAsync<T>(this MarketDataApiClient client, Func<MarketDataApiClient, Task<T?>> func) where T : class {
        try {
