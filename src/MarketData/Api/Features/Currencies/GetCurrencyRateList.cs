...
public class GetCurrencyRateList : IEndpoint {
    public record Query(
    ...
    private async Task<IResult> HandleAsync([AsParameters] Query query, MarketDbContext dbContext, CancellationToken cancellationToken) {
        var items = await dbContext.CurrencyPairs
            ...
            .SelectMany(x => x.Rates, (c, e) => new { c.BaseCurrencyId, c.QuoteCurrencyId, e.Date, Rate = e.Value })
