...
public class Policy : Entity<PolicyId>, IAggregateRoot {
    ...
    public decimal SumLastMathReservesInEur(bool round = false) {
        return LatestEvent?.SumMathReservesInEur(round) ?? 0;
    }

    public decimal SumLastMathReservesInPolicyCurrency(bool round = false) {
        return LatestEvent?.SumMathReservesInPolicyCurrency(round) ?? 0;
    }
