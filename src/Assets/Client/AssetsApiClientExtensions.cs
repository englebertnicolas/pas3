...
public static class AssetsApiClientExtensions {

    public static IServiceCollection AddAssetsApiClient(this IServiceCollection services, string serviceName = "api-assets") {
        services
            .AddHttpClient<AssetsApiClient>(client => {
                client.BaseAddress = new Uri($"http://{serviceName}");
            })
            .AddTypedClient((httpClient, sp) => {
                var authProvider = new AnonymousAuthenticationProvider();
                var requestAdapter = new HttpClientRequestAdapter(authProvider, httpClient: httpClient);
                return new AssetsApiClient(requestAdapter);
            });

        return services;
    }

    public static async Task<ErrorOr<T>> ToErrorOrAsync<T>(this AssetsApiClient client, Func<AssetsApiClient, Task<T?>> func) where T : class {
        try {
